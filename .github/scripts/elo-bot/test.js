...
async function phaseOne () {
  ...
  if (killer.currentWindow) killer.closeWindow(killer.currentWindow)
  await sleep(1000)
  // Ask the server (vanilla /execute if items) whether any GUI item reached the inventory or off hand.
  const leaked = []
  for (const item of ['player_head', 'gray_stained_glass_pane', 'chest', 'gold_ingot', 'barrier']) {
    for (const slots of ['container.*', 'weapon.offhand']) {
      killer.chatLog.length = 0
      await cmd(killer, `execute if items entity EloKiller ${slots} minecraft:${item}`, 700)
      if (!said(killer, 'Test failed')) leaked.push(`${item} in ${slots}: ${recent(killer)}`)
    }
  }
  record('GUI items cannot be taken out', leaked.length === 0, leaked.length ? leaked.join(' ; ') : 'no GUI item in inventory or off hand')
  // Control: the same check does see an item that is really there.
  await cmd(killer, 'give EloKiller player_head 1', 800)
  killer.chatLog.length = 0
  await cmd(killer, 'execute if items entity EloKiller container.* minecraft:player_head', 900)
  record('control: the check sees an item that is there', said(killer, 'Test passed'), recent(killer))
  await cmd(killer, 'clear EloKiller')
