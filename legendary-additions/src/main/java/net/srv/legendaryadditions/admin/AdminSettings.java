...
public record AdminSettings(
   ...
   public record Teleport(double maxDistance, int safeSearchRadius) {}
   ...
   public record ArrowShot(int gridSize, int layers, double spawnHeight, double speed, double damage,
   ...
   public static AdminSettings load(FileConfiguration config) {
      ...
      Teleport teleport = new Teleport(
            positive(config.getDouble("teleport.max-distance", 100.0), 100.0),
            clamp(config.getInt("teleport.safe-search-radius", 2), 0, 6));
      ...
      ArrowShot arrowShot = new ArrowShot(
            clamp(config.getInt("arrow-shot.grid-size", 5), 1, 15),
            clamp(config.getInt("arrow-shot.layers", 2), 1, 5),
            positive(config.getDouble("arrow-shot.spawn-height", 86.0), 86.0),
            positive(config.getDouble("arrow-shot.speed", 10.0), 10.0),
            Math.max(0.0, config.getDouble("arrow-shot.damage", 70.0)),
            Math.max(0.0, config.getDouble("arrow-shot.second-layer-damage", 80.0)),
            config.getBoolean("arrow-shot.damage-owner", false));

      String worldKey = config.getString("admin-dimension.world", "adminplugin:admin");
      return new AdminSettings(targeting, orbital, nuke, teleport, lawNuke, witherNuke, wolfRod, arrowShot, worldKey);
   }
